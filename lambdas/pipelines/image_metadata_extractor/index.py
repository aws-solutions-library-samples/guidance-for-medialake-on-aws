...
def extract_iptc_data(image_content):
    ...
    try:
        ...
        if hasattr(iptc_info, "data"):
            iptc_data = {key: str(value) for key, value in iptc_info.data.items() if value}
...
def lambda_handler(event, context):
    ...
    if not bucket or not key:
        ...
        return {
            'statusCode': 400,
            'body': 'Missing bucket or key information'
        }
    ...
    try:
        ...
        if extracted_metadata is None:
            return {
                'statusCode': 500,
                'body': 'Failed to extract metadata'
            }
