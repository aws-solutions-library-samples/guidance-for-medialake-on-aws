...
def parse_dynamodb_item(item: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key, value in item.items():
        ...
        if 'S' in value:
            ...
            result[key] = [parse_dynamodb_item(item) if 'M' in item else
                          item.get('S', item.get('N', item.get('BOOL', None)))
                          for item in value['L']]
        elif 'SS' in value:
...
def lambda_handler(event, context):
    ...
    logger.info(f"Policy synchronization completed: {success_count} succeeded, {failure_count} failed")
