...
def lambda_handler(event, context):
    """
    Lambda handler for the Cognito Pre-Token Generation trigger.
    
    This function:
    1. Extracts user information from the event
    2. Queries the authorization DynamoDB table for user's group memberships
    3. Adds group memberships and other custom claims to the token
    
    Args:
        event: Cognito Pre-Token Generation event
    ...
    event['response']['claimsOverrideDetails']['claimsToAddOrOverride']['groups'] = json.dumps(groups)
