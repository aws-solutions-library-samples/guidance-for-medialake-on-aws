            ...
            for log_group in page["logGroups"]:
                ...
                if resource_matches_prefix(log_group_name):
                    try:
                        ...
                        logger.info(f"Deleted CloudWatch log group: {log_group_name} in region {region_name}")
                        deleted_resources.append(f"{region_name}:{log_group_name}")
                    except ClientError as e:
                        logger.error(f"Error deleting log group {log_group_name} in region {region_name}: {str(e)}")
    except Exception as e:
        logger.error(f"Error listing CloudWatch log groups in region {region_name}: {str(e)}")
...
def delete_cloudwatch_log_groups() -> List[str]:
    ...
    if current_region != "us-east-1":
        us_east_1_resources = delete_log_groups_in_region(logs_client_us_east_1, "us-east-1")
                        ...
                        for mapping in mappings.get("EventSourceMappings", []):
                            try:
                                ...
                                if e.response["Error"]["Code"] != "ResourceNotFoundException":
                                    logger.error(f"Error deleting event source mapping {mapping['UUID']}: {str(e)}")
