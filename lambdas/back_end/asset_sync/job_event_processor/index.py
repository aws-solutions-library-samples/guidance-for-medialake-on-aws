        ...
        for record in event.get('Records', []):
            try:
                ...
                if 'detail' in message_body:
                    ...
                    logger.info(f"Processing S3 CloudTrail event - Source: {event_source}, DetailType: {detail_type}, EventSource: {event_source_detail}, EventName: {event_name}")
                    logger.info(f"Event detail: {json.dumps(detail, indent=2)}")
                    
                    # Record metric for event processing
                    metrics.add_metric(
                        name="JobEventProcessed",
