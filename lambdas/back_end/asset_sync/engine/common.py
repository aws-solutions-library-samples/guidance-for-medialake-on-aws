...
class AssetProcessor:
    ...
    def format_error(error_id, object_key, error_type, error_message, retry_count, job_id, bucket_name):
        ...
        return {
            ...
            "errorType": error_type.value if isinstance(error_type, enum.Enum) else error_type,
    ...
    def log_error(error_details):
        ...
        if 'ERROR_TABLE_NAME' not in os.environ:
            logger.warning("ERROR_TABLE_NAME environment variable is not set, skipping error logging")
            ...
            if inventory_ids:
                for inventory_id in set(inventory_ids):
                    try:
                        ...
                        logger.warning(f"Error checking inventory ID {inventory_id}: {str(e)}")
