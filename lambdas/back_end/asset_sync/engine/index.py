...
class AssetSyncEngine:
    ...
    def lookup_connector_details(self) -> tuple[Optional[str], Optional[list]]:
        ...
        try:
            ...
            if not connector_table_name:
                logger.warning("CONNECTOR_TABLE_NAME environment variable not set, cannot lookup connector by bucket")
                return None, None
            ...
            logger.error(f"Error looking up connector details for bucket {self.bucket_name}: {str(e)}", exc_info=True)
            return None, None
    ...
    def create_batch_operations_job(self) -> str:
        ...
        try:
            ...
            if prefixes and len(prefixes) > 1:
                ...
                logger.info(f"Creating {len(prefixes)} separate batch jobs for complete coverage")
    ...
    def _create_multiple_prefix_jobs(self, ingest_queue_url: Optional[str], prefixes: list) -> str:
        """Create separate batch jobs for each prefix"""
        batch_job_ids = []
        failed_prefixes = []
        
        for i, prefix in enumerate(prefixes):
            logger.info(f"Creating batch job {i+1}/{len(prefixes)} for prefix: {prefix}")
            ...
            try:
                ...
                logger.error(f"Failed to create batch job for prefix {prefix}: {str(e)}")
                failed_prefixes.append(prefix)
                continue
        
        if not batch_job_ids:
            raise Exception(f"Failed to create any batch jobs for prefixes: {failed_prefixes}")
        
        if failed_prefixes:
            logger.warning(f"Some batch jobs failed to create for prefixes: {failed_prefixes}")
        ...
        logger.info(f"Successfully created {len(batch_job_ids)} batch jobs: {batch_job_ids}")
        return batch_job_ids[0]  # Return first job ID for compatibility

    def _create_single_batch_job(self, ingest_queue_url: Optional[str], prefix: Optional[str] = None, 
                                job_suffix: str = "", description_suffix: str = "") -> str:
        ...
        logger.info(f"S3 Batch Operations create_job parameters: {json.dumps(job_params, indent=2, default=str)}")
    ...
    def _handle_single_job_completion(self, batch_job_id: str, job_status: str, metadata: dict) -> dict:
    ...
    def _handle_multi_job_completion(self, batch_job_id: str, job_status: str, metadata: dict) -> dict:
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if 'detail' in event and event.get('source') == 'aws.s3control':
            ...
            if is_multi_job:
                return engine._handle_multi_job_completion(batch_job_id, job_status, metadata)
            else:
                return engine._handle_single_job_completion(batch_job_id, job_status, metadata)
