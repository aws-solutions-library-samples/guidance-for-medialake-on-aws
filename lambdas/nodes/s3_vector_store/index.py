...
def detect_content_type(payload: Dict[str, Any], embedding_data: Dict[str, Any] = None) -> str:
    ...
    if (payload.get("map", {}).get("item", {}).get("start_time") is not None and
        payload.get("map", {}).get("item", {}).get("end_time") is not None):
        return "audio"
...
def _get_segment_bounds(payload: Dict[str, Any]) -> Tuple[int, int]:
    ...
    for i, c in enumerate(candidates):
        ...
        logger.info(f"Candidate {i}: start={start}, end={end}, keys={list(c.keys()) if c else 'None'}")
    ...
    logger.warning(f"Segment bounds not found in {len(candidates)} candidates – defaulting to 0-0")
    return 0, 0
        ...
        for i, vd in enumerate(vectors_data):
            ...
            if content_type == "audio":
                ...
                if start_sec is None or end_sec is None:
                    raise ValueError(f"Audio embedding at index {i} missing start/end offset seconds")
                ...
                end_sec = meta.get("end_offset_sec")
                if start_sec is None or end_sec is None:
                    raise ValueError(f"Video clip embedding at index {i} missing start/end offset seconds")
...
def process_store_action(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if scope in {"clip", "audio", "image"} or is_audio_content:
        ...
        if not embedding_data:
            raise ValueError("Missing 'data' or 'item' field in payload for single embedding processing")
