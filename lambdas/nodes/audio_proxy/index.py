import os, json, importlib.util, time, random, decimal
...
def create_job_with_retry(mc_client, job_settings: Dict[str, Any], max_retries: int = 5) -> Dict[str, Any]:
    attempt = 0
    while True:
        try:   return mc_client.create_job(**job_settings)
        except ClientError as e:
            if e.response["Error"]["Code"] == "TooManyRequestsException" and attempt < max_retries:
                attempt += 1
...
def load_and_execute_function_from_s3(bucket: str, key: str, fn_name: str, event: Dict[str, Any]) -> Any:
    code = s3_client.get_object(Bucket=bucket, Key=f"api_templates/{key}")["Body"].read().decode()
    spec = importlib.util.spec_from_loader("dynamic_module", loader=None)
    mod  = importlib.util.module_from_spec(spec);  exec(code, mod.__dict__)
    if not hasattr(mod, fn_name): raise AttributeError(f"{fn_name} not found in {key}")
...
def create_request_body(tmpl_paths: Dict[str, str], tmpl_bucket: str, event: Dict[str, Any]) -> Dict[str, Any]:
    tmpl = download_s3_object(tmpl_bucket, f"api_templates/{tmpl_paths['request_template']}")
    mapping = load_and_execute_function_from_s3(tmpl_bucket, tmpl_paths["mapping_file"], "translate_event_to_request", event)
...
def create_response_output(tmpl_paths: Dict[str, str], tmpl_bucket: str, resp: Dict[str, Any], event: Dict[str, Any]) -> Dict[str, Any]:
    tmpl = download_s3_object(tmpl_bucket, f"api_templates/{tmpl_paths['response_template']}")
    mapping = load_and_execute_function_from_s3(tmpl_bucket, tmpl_paths["response_mapping_file"], "translate_event_to_request", {"response_body": resp, "event": event})
    env = Environment(loader=FileSystemLoader("/tmp/"));  env.filters["jsonify"] = json.dumps
    return json.loads(env.from_string(tmpl).render(variables=mapping))
...
def _extract_asset(event: Dict[str, Any]) -> Dict[str, Any]:
    ...
    assets: List[dict] = event.get("payload", {}).get("assets", []);  _raise("Event missing assets list") if not assets else None
    event["input"] = assets[0];  return assets[0]
...
def get_mediaconvert_endpoint() -> str:
    ...
    for attempt in range(60):
        try: return mc.describe_endpoints()["Endpoints"][0]["Url"]
            ...
            logger.warning("describe_endpoints throttled, retrying in %.2fs", delay);  time.sleep(delay)
...
def lambda_handler(event: Dict[str, Any], _: LambdaContext) -> Dict[str, Any]:
    try:
        ...
        output_bucket = event.get("output_bucket") or os.environ.get("MEDIA_ASSETS_BUCKET_NAME") or _raise("MEDIA_ASSETS_BUCKET_NAME env-var missing")
        if not all([bucket, key]): _raise("Missing S3 location details in event")
        ...
        updated_item = asset_table.get_item(Key={"InventoryID": clean_inventory_id}).get("Item", {})
