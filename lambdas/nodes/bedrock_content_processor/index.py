import os, json, datetime, base64, mimetypes, importlib.util
...
def create_request_body(tpl_paths, bucket, event):
    try:
        ...
        tmpl = s3.Object(bucket, tpl_paths["request_template"]).get()["Body"].read().decode("utf-8")
...
def create_response_output(tpl_paths, bucket, result, event, mapping):
    try:
        ...
        resp_map = mod.translate_event_to_request({"response_body": result, "event": event})
        tmpl = s3.Object(bucket, tpl_paths["response_template"]).get()["Body"].read().decode("utf-8")
        env = Environment(loader=FileSystemLoader("/"), autoescape=True)
...
def lambda_handler(event, context):
    try:
        ...
        if content_src == "transcript":
            uri = (asset_detail.get("TranscriptionS3Uri")
            ...
            body_json = build_bedrock_body(model_id, instr, {"b64": b64, "mime": mime}).encode("utf-8")
