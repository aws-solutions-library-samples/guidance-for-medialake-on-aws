    ...
    def settings_system_metadata_fields_put():
        ...
        for i, field in enumerate(fields):
            if not isinstance(field, dict):
                raise BadRequestError(
                    f"Field at index {i}: each field must be a JSON object"
                )
            ...
            if not re.match(r"^[A-Za-z][A-Za-z0-9_.]{0,255}$", field["name"]):
                raise BadRequestError(
                    f"Field at index {i}: 'name' must start with a letter and contain only letters, digits, dots, or underscores (max 256 chars)"
                )
            ...
            # Preserve keyword/aggregation mapping metadata for string fields
            if field["type"] == "string":
                if field.get("keywordName"):
                    sanitized_entry["keywordName"] = field["keywordName"]
                if "aggregatable" in field:
                    sanitized_entry["aggregatable"] = bool(field["aggregatable"])

            if (
                isinstance(field.get("predefinedValues"), list)
                and field["predefinedValues"]
            ):
                sanitized_entry["predefinedValues"] = [
                    str(v)
                    for v in field["predefinedValues"]
                    if isinstance(v, (str, int, float))
                ]

            sanitized_fields.append(sanitized_entry)

        # Optional optimistic-lock token sent by the client.  When
        # present the write is conditional: it only succeeds if the
        # stored ``updatedAt`` still matches, preventing silent
        ...
        try:
            ...
            if expected_updated_at is not None:
                # Conditional write: only succeed when the stored
                # version matches what the caller last read.
                put_kwargs["ConditionExpression"] = Attr("updatedAt").eq(
                    expected_updated_at
                )
            else:
                # First-ever write or legacy client without the token:
                # allow the write only when the item doesn't exist yet
                # OR proceed unconditionally (backwards-compatible).
                # We keep backwards compatibility by not adding a
                # condition when the token is absent.
                pass
            ...
            try:
                ...
                raise ServiceError(
                    status_code=409,
                    msg=(
                        "Conflict: the metadata fields configuration was modified by "
