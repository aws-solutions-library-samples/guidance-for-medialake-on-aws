...
import functools
import traceback
from typing import Any, Dict, Callable, Optional, TypeVar, Union, cast, List
...
def check_response_status(
    ...
    if status not in success_codes:
        ...
        logger.error(f"{service} {operation} failed", extra={
            "status": status,
            "error": error_msg,
            "service": service,
            "operation": operation,
...
def handle_api_response(
    ...
    Args:
        ...
        success_codes: List of status codes considered successful (default: [200, 201, 202, 204])
    ...
    if status_code not in success_codes:
        ...
        logger.error(f"{api_name} API call to {endpoint} failed", extra={
            "status_code": status_code,
            "api_name": api_name,
            "endpoint": endpoint,
...
def with_error_handling(func: Callable[..., R]) -> Callable[..., R]:
    ...
    def wrapper(event: Dict[str, Any], context: LambdaContext, *args: Any, **kwargs: Any) -> R:
        try:
            ...
            if isinstance(result, dict) and 'statusCode' in result and result['statusCode'] not in [200, 201, 202, 204]:
                logger.warning("Lambda returned non-success status code", extra={
            ...
            logger.exception(f"Lambda error: {e.message}", extra={
            ...
            error_type = e.__class__.__name__
            logger.exception(f"Unhandled error: {error_type}", extra={
