...
def register_routes(app, dynamodb, table_name):
    ...
    def add_collection_item(collection_id: str):
        """
        Add item(s) to collection with support for bulk operations and clips.

        New format (bulk mode):
        {
            "items": [
                {"assetId": "123", "clips": [{"startTime": "...", "endTime": "..."}]},
                {"assetId": "456"},
                ...
            ]
        }
        ...
        {
            "assetId": "12345",
            "clipBoundary": {"startTime": "HH:MM:SS:FF", "endTime": "HH:MM:SS:FF"},
            "addAllClips": true
        }
        """
        try:
            ...
            # Check for new bulk format
            items_array = request_data.get("items")

            if items_array:
                # NEW PATH: Bulk mode with items array
                logger.info(
                    f"[ADD_ITEM] Processing bulk add with {len(items_array)} asset(s)"
                )
                items_to_add = []

                for item_data in items_array:
                    asset_id = item_data.get("assetId")
                    clips = item_data.get("clips", [])

                    if clips:
                        # Asset with clips - add each clip separately
                        for clip in clips:
                            clip_boundary = {
                                "startTime": clip["startTime"],
                                "endTime": clip["endTime"],
                            }
                            sk = generate_asset_sk(asset_id, clip_boundary)
                            items_to_add.append(
                                {
                                    "SK": sk,
                                    "assetId": asset_id,
                ...
                # OLD PATH: Single item mode (backward compatibility)
                ...
                if add_all_clips and not clip_boundary.get("startTime"):
                    ...
                    logger.info(
                        f"[ADD_ITEM] Adding all clips for asset {asset_id} (deprecated path)"
                    )
