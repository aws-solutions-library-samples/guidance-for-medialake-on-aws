...
def _delete_group_with_rollback(
    ...
    try:
        ...
        try:
            ...
            metrics.add_metric(name="CognitoGroupDeleted", unit=MetricUnit.Count, value=1)
        except ClientError as e:
            error_code = e.response['Error']['Code']
            if error_code == 'ResourceNotFoundException':
                logger.warning(f"Cognito group not found (may have been deleted already): {group_id}")
                # Continue with DynamoDB deletion even if Cognito group doesn't exist
            else:
                logger.error(f"Failed to delete Cognito group: {str(e)}")
                metrics.add_metric(name="CognitoGroupDeletionError", unit=MetricUnit.Count, value=1)
                raise
        ...
        logger.info(f"Successfully deleted group and {len(items)} related items from DynamoDB", 
                   extra={"group_id": group_id})
        ...
        if cognito_deleted and dynamodb_backup:
            try:
                ...
                if group_metadata:
                    ...
                    logger.error(f"Could not find group metadata for rollback: {group_id}")
