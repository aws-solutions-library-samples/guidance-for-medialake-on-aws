...
class GroupRequest(BaseModel):
    ...
    id: str = Field(..., description="Unique identifier for the group (used for Cognito group name)")
    ...
    assignedPermissionSets: Optional[List[str]] = Field(default=[], description="List of permission sets assigned to the group")
...
def lambda_handler(
    ...
    try:
        ...
        if not auth_table_name:
            ...
            return _create_error_response(500, "Internal configuration error - missing table name")
        ...
        if not cognito_user_pool_id:
            ...
            return _create_error_response(500, "Internal configuration error - missing user pool ID")
...
def _create_group_with_rollback(
    ...
    try:
        ...
        try:
            ...
            metrics.add_metric(name="CognitoGroupCreated", unit=MetricUnit.Count, value=1)
        except ClientError as e:
            error_code = e.response['Error']['Code']
            if error_code == 'GroupExistsException':
                logger.error(f"Cognito group already exists: {group_request.id}")
                metrics.add_metric(name="CognitoGroupExistsError", unit=MetricUnit.Count, value=1)
                raise ValueError(f"Group with ID '{group_request.id}' already exists")
            else:
                logger.error(f"Failed to create Cognito group: {str(e)}")
                metrics.add_metric(name="CognitoGroupCreationError", unit=MetricUnit.Count, value=1)
                raise
        ...
        try:
            ...
            logger.info(f"Successfully created DynamoDB entry for group: {group_request.id}")
            metrics.add_metric(name="DynamoDBGroupCreated", unit=MetricUnit.Count, value=1)
        except ClientError as e:
            error_code = e.response['Error']['Code']
            if error_code == 'ConditionalCheckFailedException':
                logger.error(f"DynamoDB group already exists: {group_request.id}")
                metrics.add_metric(name="DynamoDBGroupExistsError", unit=MetricUnit.Count, value=1)
                raise ValueError(f"Group with ID '{group_request.id}' already exists in database")
            else:
                logger.error(f"Failed to create DynamoDB entry: {str(e)}")
                metrics.add_metric(name="DynamoDBGroupCreationError", unit=MetricUnit.Count, value=1)
                raise
        ...
        if dynamodb_created:
            try:
                logger.info(f"Rolling back DynamoDB entry for group: {group_request.id}")
                table = dynamodb.Table(table_name)
                table.delete_item(
                    Key={
                        "PK": f"GROUP#{group_request.id}",
                ...
                logger.info(f"Successfully rolled back DynamoDB entry for group: {group_request.id}")
                metrics.add_metric(name="DynamoDBRollbackSuccess", unit=MetricUnit.Count, value=1)
            except Exception as rollback_error:
                logger.error(f"Failed to rollback DynamoDB entry: {str(rollback_error)}")
                metrics.add_metric(name="DynamoDBRollbackError", unit=MetricUnit.Count, value=1)
        ...
        if cognito_created:
            try:
                ...
                logger.info(f"Successfully rolled back Cognito group: {group_request.id}")
                metrics.add_metric(name="CognitoRollbackSuccess", unit=MetricUnit.Count, value=1)
            except Exception as rollback_error:
                logger.error(f"Failed to rollback Cognito group: {str(rollback_error)}")
                metrics.add_metric(name="CognitoRollbackError", unit=MetricUnit.Count, value=1)
