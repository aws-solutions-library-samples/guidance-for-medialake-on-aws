...
def get_opensearch_client() -> OpenSearch:
    """Create and return an OpenSearch client with refreshed IAM SigV4 auth.

    Credentials are fetched from a new boto3 Session on every call so that
    ...
    auth = RequestsAWSV4SignerAuth(credentials, region, service_scope)
...
def handle_get_fields():
    ...
    try:
        table_name = os.environ["SYSTEM_SETTINGS_TABLE_NAME"]
