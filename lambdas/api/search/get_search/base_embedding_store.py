...
class BaseEmbeddingStore(ABC):
    ...
    def generate_text_embedding(self, query_text: str) -> List[float]:
        ...
        self.logger.info(f"[PERF] Starting centralized embedding generation for query: {query_text}")
        ...
        try:
            ...
            self.logger.info(f"[PERF] Starting embedding creation for query: {query_text}")
