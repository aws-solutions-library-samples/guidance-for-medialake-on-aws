...
class PermissionSetAssignmentRequest(BaseModel):
    ...
    permissionSetIds: List[str] = Field(..., description="List of permission set IDs to assign to the user")
...
def lambda_handler(
    ...
    try:
        ...
        if not _verify_permission_sets_exist(auth_table_name, permission_set_ids):
            ...
            return _create_error_response(400, "One or more permission sets do not exist")

        # Verify that the user exists
        if not _verify_user_exists(auth_table_name, user_id):
            logger.error(f"User {user_id} does not exist")
            metrics.add_metric(
                name="InvalidUserError", unit=MetricUnit.Count, value=1
            )
        ...
        logger.info("Successfully assigned permission sets to user", 
...
def _verify_permission_sets_exist(table_name: str, permission_set_ids: List[str]) -> bool:
    """
