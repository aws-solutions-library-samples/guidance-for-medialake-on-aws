...
def lambda_handler(
    ...
    try:
        ...
        if not _check_assignment_exists(auth_table_name, user_id, permission_set_id):
            logger.error(f"Assignment does not exist for user {user_id} and permission set {permission_set_id}")
            metrics.add_metric(
                name="AssignmentNotFoundError", unit=MetricUnit.Count, value=1
            )
            return _create_error_response(404, f"Assignment not found for user {user_id} and permission set {permission_set_id}")
...
def _check_assignment_exists(table_name: str, user_id: str, permission_set_id: str) -> bool:
    """
...
@tracer.capture_method
def _remove_user_assignment(table_name: str, user_id: str, permission_set_id: str) -> None:
    """
