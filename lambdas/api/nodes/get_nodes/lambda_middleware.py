...
class LambdaMiddleware:
    ...
    def _make_output(
        ...
        # Extract external job fields (use .get() instead of safe_pop to preserve values for templates)
        ...
        if isinstance(data, dict):
            ...
            # If result is not a dict (e.g., a list), get external job fields from original metadata only
        ...
        # ── FIRST-STEP RESET ───────────────────────────────────────────────────
        # For first steps, only reset external job fields if they weren't generated by this step's processing
        if self.is_first:
            # If this step generated external job fields (from the response), preserve them
            # Only reset if no external job fields were generated by this step
            if not (ext_id or ext_st or ext_rs):
                self.logger.info("First step with no generated external job fields - resetting to null", extra={
                    "is_first": self.is_first,
                    "service": self.service,
                    "step_name": self.step_name
                })
                ext_id = None          # will become JSON null
                ext_st = None
                ext_rs = None
            else:
                self.logger.info("First step with generated external job fields - preserving them", extra={
                    "is_first": self.is_first,
                    "service": self.service,
                    "step_name": self.step_name,
                    "generated_ext_id": ext_id,
                    "generated_ext_st": ext_st,
                    "generated_ext_rs": ext_rs
                })
        else:
            self.logger.info("Not first step - preserving external job fields", extra={
                "is_first": self.is_first,
                "service": self.service,
                "step_name": self.step_name,
                "final_ext_id": ext_id,
