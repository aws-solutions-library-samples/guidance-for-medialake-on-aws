...
def validate_super_admin_access(event: Dict[str, Any]) -> str:
    ...
    try:
        ...
        logger.info(f"DEBUG: Raw claims string: {claims_str[:200]}...")  # First 200 chars
        
        try:
            claims = json.loads(claims_str) if isinstance(claims_str, str) else claims_str
            logger.info(f"DEBUG: Parsed claims keys: {list(claims.keys()) if isinstance(claims, dict) else 'not a dict'}")
        ...
        logger.info(f"DEBUG: Raw user_groups from cognito:groups: {user_groups}, type: {type(user_groups)}")
        
        if isinstance(user_groups, str):
            # Sometimes groups might be a JSON string
            try:
                user_groups = json.loads(user_groups)
                logger.info(f"DEBUG: Parsed user_groups from JSON string: {user_groups}")
            except json.JSONDecodeError:
                user_groups = [user_groups]
                logger.info(f"DEBUG: Wrapped user_groups as list: {user_groups}")
        
        # Get user email from claims
        user_email = claims.get('email', claims.get('cognito:username', authorizer.get('username', 'unknown')))
        
        logger.info(f"Authorization check for user: {user_email}, groups: {user_groups}")
        ...
        if 'superAdministrators' not in user_groups:
            ...
            raise UnauthorizedError("Access denied. Requires superAdministrators group membership.")
