...
class GitHubClient:
        ...
        try:
            ...
            for branch in branches:
                ...
                result.append({
                    ...
                    'date': self._parse_date(commit_info.get('commit', {}).get('committer', {}).get('date')),
    ...
    def get_tags(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            latest_release = None
            try:
                latest_release = self._make_request(f"/repos/{self.owner}/{self.repo}/releases/latest")
            ...
            for i, tag in enumerate(tags):
                ...
                if commit_sha:
                    try:
                        commit_details = self._make_request(f"/repos/{self.owner}/{self.repo}/commits/{commit_sha}")
                    except Exception:
                ...
                result.append({
                    ...
                    'type': 'tag',
                    'sha': commit_sha or '',
                    'date': self._parse_date(commit_details.get('commit', {}).get('committer', {}).get('date')),
                    'message': commit_details.get('commit', {}).get('message'),
                    'is_latest': tag['name'] == latest_tag or (i == 0 and latest_tag is None)
                })
