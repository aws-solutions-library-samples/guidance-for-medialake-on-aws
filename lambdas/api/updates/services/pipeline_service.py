...
class PipelineService:
                ...
                if f"{resource_prefix}" in stack['StackName'].lower():
                    ...
                    for resource in resources.get('StackResourceSummaries', []):
                        if resource['ResourceType'] == 'AWS::CodePipeline::Pipeline':
                            ...
                            logger.info(f"Discovered pipeline name: {self.pipeline_name}")
                            return self.pipeline_name
    ...
    def trigger_upgrade(
        ...
        try:
            ...
            logger.info(f"Triggering pipeline {pipeline_name} for version {target_version} ({version_type})")
            ...
            for stage in pipeline['stages']:
                if stage['name'] == 'Source':
                    for action in stage['actions']:
                        if action['actionTypeId']['provider'] in ['GitHub', 'CodeStarSourceConnection', 'S3']:
                            ...
                            if 'configuration' in action:
                                ...
                                if 'Branch' in config:
                                    ...
                                    logger.info(f"Updated source branch to: {target_version}")
                                elif 'BranchName' in config:
                                    config['BranchName'] = target_version
                                    logger.info(f"Updated source branch name to: {target_version}")
                                ...
                                if version_type == 'tag' and 'Branch' in config:
                                    ...
                                    logger.info(f"Updated source to tag: {target_version}")
    ...
    def get_pipeline_execution_status(
        ...
        try:
            ...
            return {
                ...
                'start_time': execution.get('startTime', datetime.now(timezone.utc)).isoformat(),
                'progress': progress
            }
            ...
            for stage in stages:
                ...
                if stage_status == 'Succeeded':
                    ...
                    for action in stage.get('actionStates', []):
                        if action.get('latestExecution', {}).get('status') == 'InProgress':
