...
from models.responses import UpgradeStatusResponseData, ActiveUpgrade, UpgradeProgress
...
def handle_get_status() -> Dict[str, Any]:
    ...
    try:
        ...
        if active_upgrade:
            ...
            try:
                ...
                if pipeline_execution_id:
                    pipeline_status = pipeline_service.get_pipeline_execution_status(pipeline_execution_id)
                    ...
                    active_upgrade_info = ActiveUpgrade(
                        ...
                        progress=UpgradeProgress(
                            ...
                            current_action=progress_info.get('current_action', 'Processing')
                        )
                    )
                    
                    logger.info(f"Active upgrade progress: {progress_info.get('percentage', 0)}%")
                else:
        ...
        logger.info(f"  → ✓ handle_get_status completed: status={upgrade_status}, version={current_version}")
