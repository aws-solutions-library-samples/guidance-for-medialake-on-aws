...
from models.responses import VersionsResponseData, GitHubVersion
...
def handle_get_versions() -> Dict[str, Any]:
    ...
    try:
        ...
        repo_url = os.environ.get('GITHUB_REPO_URL', 'https://github.com/aws-solutions-library-samples/guidance-for-medialake-on-aws')
        ...
        for i, branch_data in enumerate(branches_data):
            logger.debug(f"    - Processing branch {i+1}/{len(branches_data)}: {branch_data.get('name')}")
        ...
        for i, tag_data in enumerate(tags_data):
            logger.debug(f"    - Processing tag {i+1}/{len(tags_data)}: {tag_data.get('name')}")
            tag = GitHubVersion(
                name=tag_data['name'],
                type=tag_data['type'],
                sha=tag_data['sha'],
        ...
        response_data = VersionsResponseData(
            branches=branches,
            tags=tags
        )
        logger.info("  → ✓ Response data created")
        
        logger.info(f"  → ✓ handle_get_versions completed: {len(branches)} branches, {len(tags)} tags")
