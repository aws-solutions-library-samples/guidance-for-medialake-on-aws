...
def handle_post_trigger(request_body: Dict[str, Any], user_email: str) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if not request.confirm_upgrade:
            raise BadRequestError("Upgrade confirmation is required. Set 'confirm_upgrade' to true.")
        
        logger.info(f"Triggering upgrade to {request.target_version} ({request.version_type}) for user: {user_email}")
        ...
        if active_upgrade:
            logger.warning(f"Upgrade conflict detected. Active upgrade: {active_upgrade.get('upgrade_id')}")
        ...
        current_version = current_version_info.get('version', 'unknown') if current_version_info else 'unknown'
        
        logger.info(f"Current version: {current_version}, Target version: {request.target_version}")
        
        # Check if trying to upgrade to the same version
        if current_version == request.target_version:
            logger.warning(f"Attempted to upgrade to current version: {current_version}")
            raise BadRequestError(
