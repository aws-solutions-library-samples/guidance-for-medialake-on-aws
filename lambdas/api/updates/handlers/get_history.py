...
from models.responses import UpgradeRecord, PaginationInfo
...
def handle_get_history(query_params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"Fetching upgrade history with limit: {request.limit}, cursor: {request.cursor}")
        ...
        logger.info(f"Successfully fetched {len(upgrade_history)} upgrade history records")
        
        return create_success_response(
            [record.model_dump() for record in upgrade_history],
