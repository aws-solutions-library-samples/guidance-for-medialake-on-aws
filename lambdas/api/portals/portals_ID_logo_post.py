"""POST /settings/portals/{id}/logo — Upload portal logo."""
...
logger = Logger(
    service="portals-id-logo-post", level=os.environ.get("LOG_LEVEL", "INFO")
)
tracer = Tracer(service="portals-id-logo-post")


def register_route(app):
    @app.post("/settings/portals/<portal_id>/logo")
    @tracer.capture_method
    def portals_id_logo_post(portal_id: str):
        ...
        try:
            ...
            old_s3_key = getattr(item, "logoS3Key", None)

            # Upload to S3: upload-portals/{slug}/logo_{ts}.{ext}
            ...
            try:
                item.update(actions=[PortalMetadataModel.logoS3Key.set(result.s3_key)])
            except Exception:
                logger.warning("Metadata update failed after S3 upload, cleaning up")
                delete_s3_object(result.s3_key)
                raise

            # Clean up the previous logo object now that the new one is live
            ...
            logger.exception("Error uploading logo", exc_info=e)
            return create_error_response(
