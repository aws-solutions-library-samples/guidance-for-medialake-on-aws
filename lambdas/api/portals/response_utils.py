...
def create_success_response(
    ...
    """
    Create standardized success response as a Powertools Response.
    ...
    Returns:
        Powertools Response with correct status code
    """
    body = {"success": True, "data": data, "meta": create_meta(request_id)}
...
def create_error_response(
    ...
    """
    Create standardized error response as a Powertools Response.
...
def encode_cursor(payload: Dict[str, Any]) -> str:
    ...
    Raises:
        Exception: Any exception raised while serializing or encoding the
            payload is re-raised to the caller. Silently returning an
            empty string would produce broken pagination responses without
            any signal that encoding failed.
    """
    try:
        json_str = json.dumps(payload, separators=(",", ":"))
        return base64.urlsafe_b64encode(json_str.encode()).decode()
    except Exception:
        logger.exception("Error encoding cursor")
        raise
