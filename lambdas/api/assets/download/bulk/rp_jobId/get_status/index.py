...
def get_job_details(job_id: str, user_id: str) -> Dict[str, Any]:
    ...
    """
    try:
        # Query user table for bulk download job
        ...
        if not response.get("Items"):
            raise BulkDownloadError(f"Job {job_id} not found for user {user_id}", 404)
...
def get_user_jobs(user_id: str, limit: int = 10, next_token: str = None) -> Dict[str, Any]:
    """
    Retrieve all bulk download jobs for a user from the user table.
    ...
    try:
        ...
        logger.error(
            "Failed to retrieve user jobs from user table",
            extra={
                "error": str(e),
                "userId": user_id,
            },
...
def lambda_handler(event: APIGatewayProxyEvent, context: LambdaContext) -> Dict[str, Any]:
    ...
    try:
        ...
        if path.endswith("/user"):
            ...
            # Get job details (user_id is already verified by querying user table)
