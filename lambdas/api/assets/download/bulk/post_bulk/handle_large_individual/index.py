...
@tracer.capture_method
def update_job_with_large_file_urls(user_id: str, job_id: str, large_file_urls: List[str]) -> None:
    """
...
def lambda_handler(event: Dict[str, Any], context: LambdaContext) -> Dict[str, Any]:
    ...
    try:
        ...
        if not large_files:
            logger.info("No large files to process", extra={"userId": user_id, "jobId": job_id})
            return {
                "jobId": job_id,
                "userId": user_id,
                "largeFileUrls": [],
