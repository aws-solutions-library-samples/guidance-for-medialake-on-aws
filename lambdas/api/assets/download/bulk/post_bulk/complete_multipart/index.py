...
metrics = Metrics(namespace="BulkDownloadService", service="bulk-download-complete-multipart")
...
@tracer.capture_method
def get_completed_parts_from_s3(manifest_key: str, completed_parts_info: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
...
@tracer.capture_method
def get_completed_parts(event: Dict[str, Any], manifest_key: str) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        if completed_parts_info:
            ...
            logger.error(
                "No completed parts found in event",
                extra={"manifestKey": manifest_key}
            )
...
@tracer.capture_method
def complete_multipart_upload(s3_key: str, upload_id: str, parts: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
...
@tracer.capture_method
def update_job_completed_structured(user_id: str, job_id: str, download_urls: Dict[str, Any]) -> None:
    """
...
def cleanup_temp_files(job_id: str) -> None:
    ...
    try:
        ...
        if os.path.exists(job_dir):
            ...
            logger.info(
                "Removed job directory",
                extra={"jobDir": job_dir}
            )
    ...
    try:
        ...
        if completed_parts:
            ...
            for batch_result in batch_results:
                ...
                elif isinstance(batch_result, dict) and "completedParts" in batch_result:
        ...
        response = complete_multipart_upload(s3_key, upload_id, parts)
        ...
        # Add metrics
        metrics.add_metric(name="MultipartUploadCompletionErrors", unit=MetricUnit.Count, value=1)
