        ...
        if "zippedFiles" in download_urls:
            ...
            if zip_url and TEMP_BUCKET in zip_url:
                ...
                try:
                    ...
                    elif f"s3.{zip_url.split('.')[1]}.amazonaws.com/{TEMP_BUCKET}/" in zip_url:
                    ...
                    logger.warning(f"Failed to parse S3 key from URL {zip_url}: {str(e)}")
        
        # Handle individual files if they're in temp bucket
        for file_list in ["files", "singleFiles"]:
            if file_list in download_urls and isinstance(download_urls[file_list], list):
                for file_url in download_urls[file_list]:
                    if file_url and TEMP_BUCKET in file_url:
                        try:
                            if f"{TEMP_BUCKET}.s3." in file_url:
                                key = file_url.split(f"{TEMP_BUCKET}.s3.")[1].split("/", 1)[1]
                            elif f"s3.{file_url.split('.')[1]}.amazonaws.com/{TEMP_BUCKET}/" in file_url:
                            ...
                            logger.warning(f"Failed to parse S3 key from URL {file_url}: {str(e)}")
    ...
    if job_id:
        try:
            ...
            response = s3_client.list_objects_v2(
                Bucket=TEMP_BUCKET,
                Prefix=f"bulk-downloads/{job_id}/",
                MaxKeys=1000
            )
            ...
            logger.warning(f"Failed to list objects with prefix bulk-downloads/{job_id}/: {str(e)}")
    ...
    if objects_to_delete:
        try:
            ...
            for i in range(0, len(objects_to_delete), 1000):
                ...
                metrics.add_metric(name="S3ObjectsDeleted", unit=MetricUnit.Count, value=deleted_count)
                if error_count > 0:
                    metrics.add_metric(name="S3DeletionErrors", unit=MetricUnit.Count, value=error_count)
...
def delete_job_record(job_id: str) -> None:
    ...
    """
    try:
        bulk_download_table.delete_item(
            Key={"jobId": job_id}
        )
...
def lambda_handler(event: Dict[str, Any], context: LambdaContext) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info("Processing job deletion", extra={"jobId": job_id})
