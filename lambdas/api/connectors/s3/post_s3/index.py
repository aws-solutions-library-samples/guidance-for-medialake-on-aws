...
def create_resource_name_with_suffix(resource_type: str, base_name: str, suffix: str) -> str:
    """
    ...
    if len(base_name) <= available_length:
        ...
        logger.info(f"Truncated {resource_type} base name from '{base_name}' to '{truncated_base}' to accommodate suffix '{suffix}' (max: {max_length})")
        ...
        try:
            ...
            try:
                sts_client.assume_role(
                    RoleArn=role_arn,
                    RoleSessionName="test-lambda-propagation"
                )
...
def attach_policy_async(iam_client, role_name, policy_name, policy_document, created_resources):
...
def setup_eventbridge_notifications(
    s3_bucket: str, bucket_region: str, created_resources: list, object_prefix: list[str] | None, suffix: str
) -> tuple[str, str]:
    ...
    queue_name = create_resource_name_with_suffix("sqs_queue_fifo", base_name, suffix) + ".fifo"
    ...
    # Get account ID
    account_id = get_optimized_client("sts", bucket_region).get_caller_identity()["Account"]

    # Create EventBridge rule with comprehensive event pattern
    rule_name_base = f"medialake-{s3_bucket}-s3-events"
    rule_name = create_resource_name_with_suffix("eventbridge_rule", rule_name_base, suffix)
    event_pattern = {
        "source": ["aws.s3"],
        "detail-type": [
            "Object Created",
    ...
    target_id = create_resource_name_with_suffix("eventbridge_target_id", target_id_base, suffix)
...
def create_eventbridge_role(
    bucket_region: str, queue_arn: str, rule_name: str, created_resources: list, suffix: str
) -> str:
    ...
    try:
        ...
        logger.info(f"EventBridge IAM role {role_name} already exists, using existing role")
        return existing_role["Role"]["Arn"]
    except iam.exceptions.NoSuchEntityException:
        # Role doesn't exist, proceed with creation
        logger.info(f"EventBridge IAM role {role_name} does not exist, creating new role")
        pass
    ...
    try:
        ...
        logger.info(f"EventBridge IAM role {role_name} was created by another process, retrieving existing role")
        existing_role = iam.get_role(RoleName=role_name)
        return existing_role["Role"]["Arn"]
    ...
    try:
        ...
        logger.error(f"Error attaching policy to EventBridge role {role_name}: {str(e)}")
...
def create_lambda_iam_role(iam_client, role_name, kms_key_arn=None):
    ...
    try:
        ...
        # Role was created by another process while we were checking
        logger.info(f"IAM role {role_name} was created by another process, retrieving existing role")
        existing_role = iam_client.get_role(RoleName=role_name)
        return existing_role["Role"]["Arn"]
    ...
    try:
        ...
        logger.error(f"Error attaching basic execution policy to role {role_name}: {str(e)}")
...
def create_eventbridge_pipe(
    ...
    fixed_parts_length = len(resource_prefix) + len("_connector_") + len("-pipe-role-") + len(suffix)
    ...
    if len(bucket_name) > max_bucket_length:
        ...
        if max_bucket_length > 20:  # Only do smart truncation if we have reasonable space
        ...
        logger.info(f"Truncated bucket name in pipe role from '{bucket_name}' to '{truncated_bucket}' for length limits")
        pipe_role_name_base = f"{resource_prefix}_connector_{truncated_bucket}-pipe-role"
    else:
    ...
    if len(pipe_role_name) > 64:
        logger.warning(f"Pipe role name '{pipe_role_name}' still exceeds 64 chars, truncating...")
        pipe_role_name = pipe_role_name[:64]

    logger.info(f"Creating EventBridge Pipe role with name: {pipe_role_name} (length: {len(pipe_role_name)})")
    
    # Check if pipe role already exists
    try:
        existing_role = iam_client.get_role(RoleName=pipe_role_name)
        logger.info(f"Pipe IAM role {pipe_role_name} already exists, using existing role")
        ...
        logger.info(f"Creating pipe role with trust policy for pipes.amazonaws.com service")
        try:
            ...
            logger.info(f"Successfully created pipe IAM role: {pipe_role_name} with ARN: {pipe_role_arn}")
        except iam_client.exceptions.EntityAlreadyExistsException:
            # Role was created by another process while we were checking
            logger.info(f"Pipe IAM role {pipe_role_name} was created by another process, retrieving existing role")
            existing_role = iam_client.get_role(RoleName=pipe_role_name)
            pipe_role_arn = existing_role["Role"]["Arn"]
    ...
    try:
        ...
        logger.error(f"Error attaching source policy to pipe role {pipe_role_name}: {str(e)}")
        # Don't fail for policy attachment issues if role already exists
    ...
    try:
        ...
        logger.error(f"Error attaching target policy to pipe role {pipe_role_name}: {str(e)}")
        # Don't fail for policy attachment issues if role already exists
    ...
    pipe_name = create_resource_name_with_suffix("eventbridge_pipe", pipe_name_base, suffix)
    ...
    for attempt in range(max_retries):
        try:
            ...
            error_message = str(e)
            if "does not have permission to assume the role" in error_message and attempt < max_retries - 1:
                wait_time = 2 * (attempt + 1)  # Ultra-fast: 2, 4 seconds only
                logger.warning(f"Pipe creation retry {attempt + 1}/{max_retries} after {wait_time}s...")
                time.sleep(wait_time)
                continue
            else:
                logger.error(f"Failed to create EventBridge Pipe after {attempt + 1} attempts: {error_message}")
                raise e
...
def create_connector(createconnector: S3Connector) -> dict:
    ...
    try:
        ...
        if integration_method == "eventbridge":
            ...
            queue_name = create_resource_name_with_suffix("sqs_queue", queue_name_base, suffix)
            response = sqs.create_queue(
        ...
        try:
            ...
            eventbridge_policy_name = truncate_resource_name("iam_policy", eventbridge_policy_name_base)
            ...
            with ThreadPoolExecutor(max_workers=4) as executor:
                future_to_policy = {
                    executor.submit(attach_policy_async, iam_client, role_name, policy_name, policy_doc, created_resources): policy_name
                ...
                for future in as_completed(future_to_policy):
                    ...
                    try:
                        ...
                        logger.error(f"Policy attachment generated an exception for {policy_name}: {exc}")
                ...
                if policy_failures:
                    ...
                    logger.info(f"Successfully attached all policies to role {role_name}")
            ...
            if not wait_for_iam_role_propagation(iam_client, role_name):
                logger.warning(f"Role {role_name} propagation check failed, continuing anyway...")
            ...
            for lambda_attempt in range(max_lambda_retries):
                try:
                    ...
                    if "cannot be assumed by Lambda" in str(e) and lambda_attempt < max_lambda_retries - 1:
                        wait_time = 5 * (lambda_attempt + 1)  # Ultra-fast: 5, 10 seconds
                        logger.warning(f"Lambda retry {lambda_attempt + 1}/{max_lambda_retries} after {wait_time}s...")
                        time.sleep(wait_time)
                        continue
                    else:
                        logger.error(f"Lambda creation failed after {lambda_attempt + 1} attempts: {str(e)}")
                        raise
                except Exception as e:
                    logger.error(f"Unexpected error during Lambda creation (attempt {lambda_attempt + 1}): {str(e)}")
            ...
            if lambda_arn is None:
                raise Exception(f"Failed to create Lambda function {target_function_name} after {max_lambda_retries} attempts")
        ...
        try:
            ...
            logger.error(f"Error initializing cleanup clients: {str(cleanup_init_error)}")
        ...
        for resource_type, resource_id in reversed(created_resources):
            try:
                if resource_type == "eventbridge_pipe" and pipes_client:
                    ...
                    logger.info(f"Removed EventBridge target: {target_id} from rule: {rule_name}")
                    ...
                    logger.info(f"Removed EventBridge config from bucket: {resource_id}")
