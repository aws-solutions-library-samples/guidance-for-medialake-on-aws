...
def delete_environment(environment_id: str):
    try:
        # Get environment table name from environment variables
        environments_table_name = os.environ.get("ENVIRONMENTS_TABLE")
        integrations_table_name = os.environ.get("INTEGRATIONS_TABLE")

        if not environments_table_name or not integrations_table_name:
        ...
        # Check if environment is being used in any integrations
        integration_response = integrations_table.scan(
            FilterExpression="environmentId = :env_id",
            ExpressionAttributeValues={":env_id": environment_id},
        )

        if integration_response.get("Items", []):
            return Response(
                status_code=409,
                content_type=content_types.APPLICATION_JSON,
                body={
                    "status": "409",
                    "message": f"Environment {environment_id} is in use by one or more integrations and cannot be deleted",
                    "data": {
                        "integrations": [
                            item["id"] for item in integration_response["Items"]
                        ]
                    },
                },
            )

        # Delete environment from DynamoDB
        try:
            environments_table.delete_item(Key={"id": environment_id})
            logger.info(f"Successfully deleted environment {environment_id}")
            return {
                "status": "200",
            ...
            logger.exception(f"Failed to delete environment: {str(e)}")
            return Response(
                status_code=500,
                content_type=content_types.APPLICATION_JSON,
                body={
