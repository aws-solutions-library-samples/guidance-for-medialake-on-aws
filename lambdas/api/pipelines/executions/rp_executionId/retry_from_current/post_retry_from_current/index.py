...
tracer = Tracer()
metrics = Metrics(namespace="Pipelines", service="ExecutionRedriveRetry")
...
class RetryFromCurrentResponse(BaseModel):
    """Response model for retry from current execution"""
    status: str = Field(..., description="Status code of the operation")
    message: str = Field(..., description="Response message")
    redrive_date: str = Field(None, description="Date when execution was redriven")
...
@app.post("/pipelines/executions/<execution_id>/retry-from-current")
@tracer.capture_method
def handle_retry_from_current(execution_id: str) -> Dict[str, Any]:
    """
    Handle retry from current position request using redrive_execution

    Args:
        execution_id: The ID of the execution to redrive
    ...
    try:
        ...
        response = RetryFromCurrentResponse(
            status="200", 
        ...
        logger.error(f"Failed to redrive execution: {str(e)}")
