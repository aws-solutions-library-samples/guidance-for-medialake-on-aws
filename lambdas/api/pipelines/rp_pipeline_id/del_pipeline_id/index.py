...
def delete_lambda_function(function_arn: str):
    ...
    try:
        ...
        try:
            mappings = lambda_client.list_event_source_mappings(FunctionName=function_arn)
            for mapping in mappings.get('EventSourceMappings', []):
                try:
                    ...
                    if e.response['Error']['Code'] != 'ResourceNotFoundException':
                        raise
                    logger.warning(f"Event source mapping {mapping['UUID']} already deleted")
        except ClientError as e:
            if e.response['Error']['Code'] != 'ResourceNotFoundException':
                raise
    ...
    try:
        ...
        if 'triggerLambdaArn' in pipeline:
            try:
                ...
                if not isinstance(e, ClientError) or e.response['Error']['Code'] != 'ResourceNotFoundException':
        ...
        if 'queueUrl' in pipeline:
            try:
                ...
                if not isinstance(e, ClientError) or e.response['Error']['Code'] != 'AWS.SimpleQueueService.NonExistentQueue':
        ...
        if 'eventBridgeRuleArn' in pipeline:
            try:
                ...
                if not isinstance(e, ClientError) or e.response['Error']['Code'] != 'ResourceNotFoundException':
                    deletion_errors.append(f"Failed to delete EventBridge rule: {str(e)}")
        ...
        if 'roleArn' in pipeline:
            try:
                ...
                if not isinstance(e, ClientError) or e.response['Error']['Code'] != 'NoSuchEntity':
        ...
        if deletion_errors:
            logger.warning(f"Pipeline deletion completed with errors: {deletion_errors}")
            return Response(
                status_code=207,
                content_type=content_types.APPLICATION_JSON,
                body={
