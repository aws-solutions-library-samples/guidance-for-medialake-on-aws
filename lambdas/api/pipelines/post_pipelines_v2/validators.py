...
class StateMachineValidator:
    ...
    def _validate_state(self, state_name: str, state: Dict[str, Any], states: Dict[str, Any]) -> bool:
        """
    ...
    def _validate_choice_state(self, state_name: str, state: Dict[str, Any], states: Dict[str, Any]) -> bool:
        """
        ...
        if "Choices" not in state or not state["Choices"]:
            logger.error(f"Choice state {state_name} has no Choices array or it is empty")
            return False
        ...
        for i, choice in enumerate(state["Choices"]):
            ...
            if next_state not in states:
                logger.error(f"Choice {i} in state {state_name} points to non-existent state {next_state}")
                return False
                
            # Check if Variable exists
            if "Variable" not in choice:
                logger.error(f"Choice {i} in state {state_name} has no Variable property")
                return False
                
            # Check if at least one comparison operator exists
            has_comparison = False
            for op in ["StringEquals", "StringLessThan", "StringGreaterThan", "StringLessThanEquals",
            ...
            if not has_comparison:
                logger.error(f"Choice {i} in state {state_name} has no comparison operator")
                return False
        ...
        if "Default" in state:
            ...
            if default_state not in states:
                logger.error(f"Default in state {state_name} points to non-existent state {default_state}")
                return False
                
        return True
        
    def _validate_map_state(self, state_name: str, state: Dict[str, Any], states: Dict[str, Any]) -> bool:
        """
        ...
        if start_at not in iterator["States"]:
            logger.error(f"StartAt in Iterator of Map state {state_name} points to non-existent state {start_at}")
            return False
    ...
    def _validate_task_state(self, state_name: str, state: Dict[str, Any], states: Dict[str, Any]) -> bool:
        """
        ...
        # Check if Next exists and points to a valid state
        if "Next" in state and state["Next"] not in states:
            logger.error(f"Next in Task state {state_name} points to non-existent state {state['Next']}")
            return False
    ...
    def _validate_basic_state(self, state_name: str, state: Dict[str, Any], states: Dict[str, Any]) -> bool:
        """
        ...
        if state_type in ["Pass", "Wait"]:
            if "Next" in state:
                ...
                if next_state not in states:
                    logger.error(f"Next in {state_type} state {state_name} points to non-existent state {next_state}")
                    return False
            elif "End" not in state:
                logger.error(f"{state_type} state {state_name} has neither Next nor End property")
                return False
    ...
    def fix_invalid_states(self, states: Dict[str, Any], start_at: str) -> None:
                ...
                if "Choices" in state:
                    for choice in state["Choices"]:
                        if "__PLACEHOLDER__" in str(choice.get("Next", "")):
                            ...
                            valid_next = next((s for s in states.keys() if s != state_name), None)
                            if valid_next:
                                choice["Next"] = valid_next
                                logger.info(f"Fixed Choice state {state_name} Choices Next to {valid_next}")
                                
                # Fix Default
                if "__PLACEHOLDER__" in str(state.get("Default", "")):
                    # Look for a Wait state to use as Default
                    wait_state = next((s for s, st in states.items()
                                       if st.get("Type") == "Wait" and s != state_name), None)
                    if wait_state:
                        state["Default"] = wait_state
                        logger.info(f"Fixed Choice state {state_name} Default to Wait state {wait_state}")
                    else:
                        # If no Wait state, find any valid state
                        valid_default = next((s for s in states.keys() if s != state_name), None)
                        if valid_default:
                            state["Default"] = valid_default
                            logger.info(f"Fixed Choice state {state_name} Default to {valid_default}")
        
        # Fix states that have neither Next nor End properties
        for state_name, state in states.items():
            if state.get("Type") not in ["Succeed", "Fail", "Choice"] and "Next" not in state and "End" not in state:
                logger.warning(f"State {state_name} has neither Next nor End property, adding End: true")
                state["End"] = True
        ...
        for state_name, state in states.items():
            if state.get("Type") == "Map" and "Iterator" in state and "States" in state["Iterator"]:
                iterator_states = state["Iterator"]["States"]
                
                # Check if PassState exists but is not reachable
                if "PassState" in iterator_states and state["Iterator"].get("StartAt") != "PassState":
                    ...
                    if len(iterator_states) > 1:
                        ...
                        logger.info(f"Removed unreachable PassState from Map state {state_name} Iterator")
                    else:
                        # If PassState is the only state, make it the StartAt
                        state["Iterator"]["StartAt"] = "PassState"
                        logger.info(f"Made PassState reachable in Map state {state_name} Iterator")
        ...
        if not has_terminal_state and states:
            ...
            logger.info("Added a Succeed state to ensure at least one terminal state exists")
    ...
    def _find_reachable_states(self, states: Dict[str, Any], start_at: str) -> Set[str]:
        ...
        def dfs(state_name):
            ...
            if state.get("Type") == "Map" and "Next" in state and state["Next"] in states:
                dfs(state["Next"])
