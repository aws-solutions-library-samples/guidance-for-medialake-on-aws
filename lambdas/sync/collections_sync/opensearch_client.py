"""OpenSearch client for Collections Sync Lambda.
...
logger = Logger(service="collections-sync-os-client")
...
class DocumentNotFoundError(Exception):
    ...
    This typically happens when a PERM# event arrives before the parent
    collection METADATA has been indexed — a known race condition with
    DynamoDB Streams cross-partition ordering.
    """
...
def update_document(client: OpenSearch, index: str, doc_id: str, script: dict) -> None:
    ...
    as a batchItemFailure for retry, preventing silent data loss when a
    PERM# event arrives before the collection METADATA has been indexed.
