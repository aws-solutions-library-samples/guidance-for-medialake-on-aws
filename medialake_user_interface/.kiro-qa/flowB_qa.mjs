...
// STEP 4 — pipeline editor: add "Upload Batch Completed" trigger, configure
// Automation Tag (qa-auto-1782076662-tag) + Completion Outcome, add
// "Mark Upload Processing Complete" barrier downstream, connect, then Save
// (= deploy via createPipeline) and capture the API result (expect 2xx).
const TS = Date.now();
const PIPE = `qa-auto-pipe-${TS}`;
const TAG = "qa-auto-1782076662-tag"; // automation tag of the QA portal created in STEP 3

const { browser, page, consoleErrors, pageErrors } = await launch({
  viewport: { width: 1728, height: 1080 },
});
const log = (...a) => console.log("[flowBqa]", ...a);
...
page.on("response", async (res) => {
  ...
  if (/\/pipelines(\b|\/|\?)/.test(u) && m !== "GET") {
    let b = ""; try { b = (await res.text()).slice(0, 1200); } catch {}
    pipeApi.push({ method: m, status: res.status(), url: u, body: b });
  }
});

const shot = async (n) => {
  await page.screenshot({ path: `${SHOTS}/step4-${n}.png`, fullPage: false }).catch(() => {});
};
...
try {
  ...
  // Report deployed automation_tag options from /nodes.
  for (const r of nodeApi) {
    try {
      ...
      if (ubc) {
        ...
        const mm = Array.isArray(methods) ? methods[0] : methods?.trigger || Object.values(methods || {})[0];
        const params = mm?.config?.parameters || mm?.parameters || [];
        log("DEPLOYED upload_batch_completed params:", JSON.stringify(params).slice(0, 1200));
      }
    } catch (e) {
  ...
  // ---- Add TRIGGER: Upload Batch Completed ----
  ...
  if (await dialog.count()) {
    ...
    for (let i = 0; i < nCombo; i++) {
      ...
      if (/outcome|completion/i.test(labelText)) {
        ...
        const complete = page.locator('li[role="option"]', { hasText: /complete/i });
        if (await both.count()) { await both.first().click(); picked = "Both"; }
        else if (await complete.count()) { await complete.first().click(); picked = "Complete"; }
      } else if (/automation|tag|portal/i.test(labelText)) {
  ...
  // ---- Add UTILITY: Mark Upload Processing Complete ----
  if (await search.count()) {
    await search.fill("");
  ...
  if (!saveDisabled) {
    ...
    for (let i = 0; i < 40; i++) {
      ...
      const hit = ds.find((d) => /Pipeline Creation|Creating Pipeline|Pipeline Created|Failed|success|started|monitor|error/i.test(d.text));
      if (hit) { result = hit.text; if (!/Please wait|Creating/i.test(hit.text)) { log("DEPLOY modal:", JSON.stringify(hit.text)); break; } }
