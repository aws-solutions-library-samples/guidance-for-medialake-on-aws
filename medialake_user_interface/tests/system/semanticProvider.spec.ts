...
test.describe('Semantic Provider Configuration Management', () => {
  test('should navigate to System Settings page', async ({ authenticatedPage: page }) => {
    ...
    await expect(page.locator('#settings-tabpanel-0')).toBeVisible({ timeout: 10000 });
    ...
    await expect(page.getByRole('button', { name: /Edit Provider/i })).toBeVisible({ timeout: 10000 });
  });
