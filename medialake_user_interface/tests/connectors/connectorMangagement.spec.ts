...
test.describe('Connector Management', () => {
  test('should add and delete an S3 connector', async ({ authenticatedPage, s3BucketName, s3BucketDeletion }) => {
    ...
    while (!bucketFound && attempts < maxAttempts) {
      try {
        ...
        attempts++;
        console.log(`[Test] Bucket ${s3BucketName} not found in dropdown, attempt ${attempts}/${maxAttempts}`);
        if (attempts < maxAttempts) {
          ...
          throw new Error(`S3 bucket ${s3BucketName} not found in dropdown after ${maxAttempts} attempts`);
        }
      }
    }
