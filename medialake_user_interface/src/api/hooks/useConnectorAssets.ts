...
export const useConnectorAssets = ({
    ...
    const query = bucketName ? `storageIdentifier:${bucketName}${assetType ? ` type:${assetType}` : ''}` : '';
        ...
        queryFn: async ({ signal }) => {
            try {
                ...
                if (response.data?.status && !response.data.status.startsWith('2')) {
                    const error = new Error(response.data.message || 'Search request failed') as ConnectorAssetsError;
