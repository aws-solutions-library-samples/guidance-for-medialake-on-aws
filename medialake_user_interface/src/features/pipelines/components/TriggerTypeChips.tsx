        ...
        for (const [resourceType, resourceValue] of pipeline.dependentResources) {
            if (resourceType === "eventbridge_rule") {
                ...
                if (rule.ruleName) {
                    ...
                    if (ruleName.includes('default-image-pipeline') || pipeline.name.includes('Image Pipeline')) {
                    ...
                    } else if (ruleName.includes('default-video-pipeline') || pipeline.name.includes('Video Pipeline')) {
                        rule.description = "Triggers on video files (MP4, MOV, AVI, MKV, WEBM)";
                        rule.fileTypes = ["MP4", "MOV", "AVI", "MKV", "WEBM"];
                        rule.eventType = "AssetCreated";
                    } else if (ruleName.includes('default-audio-pipeline') || pipeline.name.includes('Audio Pipeline')) {
