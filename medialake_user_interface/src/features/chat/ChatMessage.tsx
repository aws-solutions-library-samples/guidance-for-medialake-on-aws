...
const detectMediaType = (content: string): { type: 'text' | 'image' | 'video' | 'audio', url?: string } => {
...
const ChatMessage: React.FC<ChatMessageProps> = ({ message }) => {
  ...
  const formattedDate = isToday ? 'Today' : new Intl.DateTimeFormat('en-US', {
    month: 'short',
    day: 'numeric'
  }).format(messageDate);
        ...
        <Box sx={{
          ...
          <Paper
            ...
            sx={{
              ...
              boxShadow: theme => `0 2px 10px ${alpha(theme.palette.common.black, theme.palette.mode === 'dark' ? 0.3 : 0.08)}`,
              overflow: 'hidden',
              position: 'relative',
            }}
          >
            ...
            {message.sender === 'system' && (message.isThinking || message.content.includes('...')) && (
              <Box sx={{
                display: 'flex',
                alignItems: 'center',
                mt: 0.5,
          ...
          >
            {formattedDate !== 'Today' && `${formattedDate}, `}{formattedTime}
          </Typography>
        </Box>
      </Box>
    </Fade>
