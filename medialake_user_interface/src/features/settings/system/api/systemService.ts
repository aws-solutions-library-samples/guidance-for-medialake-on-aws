...
export class SystemService {
  public static async getSystemSettings(): Promise<SystemSettingsResponse> {
    const response = await apiClient.get<SystemSettingsResponse>(SYSTEM_API.endpoints.GET_SYSTEM_SETTINGS);
    return response.data;
  }

  public static async getSearchProvider(): Promise<SystemSettingsResponse> {
    const response = await apiClient.get<SystemSettingsResponse>(SYSTEM_API.endpoints.GET_SEARCH_PROVIDER);
    return response.data;
  }

  public static async createSearchProvider(provider: SearchProviderCreate): Promise<SystemSettingsResponse> {
  ...
  public static async updateSearchProvider(provider: SearchProviderUpdate): Promise<SystemSettingsResponse> {
