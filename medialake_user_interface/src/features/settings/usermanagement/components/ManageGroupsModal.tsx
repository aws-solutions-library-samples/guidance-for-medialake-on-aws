...
const ManageGroupsModal: React.FC<ManageGroupsModalProps> = ({ open, onClose }) => {
  ...
  const [activeGroupForPermissionSet, setActiveGroupForPermissionSet] = useState<string | null>(null);
  ...
  const assignedPermissionSetIds = groupAssignments?.assignments?.map(a => a.permissionSetId) || [];
    ...
    <>
                  ...
                  {selectedTab === index && (
                    <Box>
                      {confirmDeleteGroup?.id === group.id ? (
                        <Card variant="outlined" sx={{ mb: 3, p: 2 }}>
                          <CardContent>
                            ...
                            <Typography>
                              {t('groups.messages.deleteWarning', { name: group.name })}
                            </Typography>
                          </CardContent>
                          <CardActions>
                            <Button 
                            ...
                            <Button 
                              ...
                              startIcon={deleteGroupMutation.isPending ? <CircularProgress size={20} /> : null}
                            >
                        ...
                        <Card variant="outlined" sx={{ mb: 3, p: 2 }}>
                          ...
                          <CardActions>
                            ...
                            <Button 
                              ...
                              startIcon={updateGroupMutation.isPending ? <CircularProgress size={20} /> : null}
                            >
                      ...
                      <Box sx={{ mt: 3 }}>
                        ...
                        {groupAssignments?.assignments && groupAssignments.assignments.length > 0 ? (
                          <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
                            {groupAssignments.assignments.map((assignment) => (
                              <Chip
                                key={assignment.permissionSetId}
                                label={assignment.permissionSetName}
                                onDelete={() => handleRemovePermissionSet(group.id, assignment.permissionSetId)}
                                sx={{ mb: 1 }}
                              />
                            ))}
                          </Stack>
      ...
      >
        {availablePermissionSets.length > 0 ? (
          ...
          <MenuItem disabled>
            {t('groups.noAvailablePermissionSets')}
          </MenuItem>
        )}
      </Menu>
