...
export const EditCollectionModal: React.FC<EditCollectionModalProps> = ({
  ...
  // Derive per-user permissions from the backend-provided userRole (single source of truth).
  // Falls back to legacy fields only when userRole is absent on older cached data.
  ...
  useEffect(() => {
    if (collection && open) {
      ...
      // Initialize metadata rows together with form reset
      setMetadataRows(
        collection.customMetadata
          ? Object.entries(collection.customMetadata).map(([key, value]) => ({ key, value }))
          : []
      );
    }
  }, [collection?.id, open]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Save form data changes (always include metadata for replace semantics)
      const updateData = {
        ...
        try {
          ...
          const msg =
            thumbnailError instanceof Error
              ...
              : "Failed to update thumbnail. Your other changes were saved.";
          setThumbnailSaveError(msg);
          return; // Stay in modal so user sees the error
        }
      }

      onClose();
    } catch {
      // Form save error — surfaced via updateCollectionMutation.isError
    }
  };
  ...
  const hasMetadataChanges = (() => {
    const original = collection?.customMetadata || {};
    const originalEntries = Object.entries(original);
    const currentValid = metadataRows.filter((r) => r.key.trim());
    if (originalEntries.length !== currentValid.length) return true;
    return currentValid.some((row) => original[row.key.trim()] !== row.value.trim());
  })();
  ...
  return (
    ...
    >
      ...
      {/* Preflight permission gate — shown when the user cannot edit this collection.
          Blocks all edit UI (including thumbnail upload) so the API is never called
          with a request the backend would reject. */}
      {collection && !canEdit && (
        <DialogContent sx={{ p: 0 }}>
          ...
          >
            ...
            >
              <PrivateIcon sx={{ fontSize: 28, color: "warning.main" }} />
            </Box>
            <Typography variant="h6" sx={{ fontWeight: 600, fontSize: "1rem" }}>
              {t(
                "collectionsPage.form.noEditPermissionTitle",
                "You do not have permission to edit this collection"
              )}
            </Typography>
            <Typography
              variant="body2"
              sx={{ color: "text.secondary", fontSize: "0.85rem", maxWidth: 360 }}
            >
              {t(
                "collectionsPage.form.noEditPermissionMessage",
                "Only the collection owner or a shared editor can make changes. Please contact the collection owner to request access."
              )}
            </Typography>
            <Box
              sx={{
        ...
        <DialogContent sx={{ p: 0, display: "flex", flexDirection: "column" }}>
          <Box sx={{ px: 3, pt: 2.5, pb: 2, display: "flex", flexDirection: "column", gap: 2.5 }}>
            ...
            {!isOwner && (
              ...
              >
                {t(
                  "collectionsPage.form.editorNotice",
                  "You are editing this collection as a shared editor. Some settings like visibility can only be changed by the owner."
                )}
              </Alert>
            )}
            ...
            <Box>
              ...
              >
                ...
                {selectedParent ? (
                  <Box
                    sx={{ flex: 1, minWidth: 0, display: "flex", alignItems: "center", gap: 0.3 }}
                  >
            ...
            {thumbnailSaveError && (
              <Alert
                severity="warning"
                variant="outlined"
                onClose={() => setThumbnailSaveError(null)}
                sx={{ borderRadius: 2, fontSize: "0.82rem" }}
              >
                {thumbnailSaveError}
              </Alert>
