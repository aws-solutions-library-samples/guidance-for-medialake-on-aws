...
const CustomFieldFilter: React.FC<{
  ...
  const allValues = React.useMemo(() => {
    ...
    // Build a map from eager buckets (global index counts, unfiltered)
    ...
    // Always use the eager (global) count so numbers stay stable regardless
    // of which cross-field filters are active.  Live aggregation counts are
    // scoped to the current query and cause confusing jumps when other
    // filters narrow the result set.
    const merged: FacetBucket[] = baseKeys.map((val) => ({
      key: val,
      doc_count: eagerMap.get(val) ?? 0,
    }));

    // Append any live buckets not already in the base set
    // (e.g. values that appeared after the eager fetch)
  ...
  // Build selected options — include user-typed values not in allValues
  const selectedOptions = React.useMemo(() => {
    const knownMap = new Map(allValues.map((v) => [v.key, v]));
    return fieldDraft.selectedFacetValues.map((key) => knownMap.get(key) ?? { key, doc_count: 0 });
  }, [allValues, fieldDraft.selectedFacetValues]);

  // Enable free-text entry when there are no pre-populated options so users
  // can type arbitrary filter values and press Enter to add them as chips.
  const hasPopulatedOptions = allValues.length > 0;

  return (
    <Box sx={{ mb: 1.5 }}>
      <Autocomplete<FacetBucket, true, false, true>
        multiple
        freeSolo
        size="small"
        options={allValues}
        ...
        onChange={(_, newValue) => {
          // newValue items can be strings (freeSolo typed) or FacetBucket objects
          const keys = newValue.map((v) => (typeof v === "string" ? v : v.key));
          onUpdate({ selectedFacetValues: keys });
        }}
        filterOptions={(options, params) => {
          const filtered = options.filter((o) =>
            o.key.toLowerCase().includes(params.inputValue.toLowerCase())
          );
          return filtered;
        }}
          ...
          <TextField
            ...
            placeholder={
              selectedOptions.length === 0
                ? hasPopulatedOptions
                  ? t("search.filters.filterBy", { field: displayName })
                  : t("search.filters.typeToAdd", "Type a value and press Enter…")
                : ""
            }
          />
        )}
        noOptionsText={
          hasPopulatedOptions
            ? t("search.filters.noValuesFound", "No values found")
            : t("search.filters.typeToAdd", "Type a value and press Enter")
        }
        sx={{ minWidth: 200 }}
      />
