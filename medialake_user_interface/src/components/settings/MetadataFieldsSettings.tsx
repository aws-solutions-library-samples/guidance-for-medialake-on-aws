...
const MetadataFieldsSettings: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Track the server's updatedAt for optimistic concurrency on save
    setLastUpdatedAt(configQuery.data?.data?.updatedAt ?? null);
  ...
  const handleRefreshFields = useCallback(async () => {
    ...
    try {
      ...
      let removedCount = 0;
      setLocalFields((prev) => {
        removedCount = Object.keys(prev).filter((k) => !newNames.has(k)).length;
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
          ...
          if (response.data?.data) {
            for (const f of fields) {
              if (f.autoPopulateValues && response.data.data[f.name]) {
                // Normalize bucket objects to string[] — the API returns
                // aggregation buckets ({key, doc_count}) not plain strings.
                const raw = response.data.data[f.name];
                f.predefinedValues = (raw as any[]).map((bucket: any) =>
                  typeof bucket === "string"
                    ? bucket
                    : typeof bucket === "object" && bucket !== null
                      ? String(bucket.key ?? bucket.value ?? bucket)
                      : String(bucket)
                );
              }
            }
      ...
      const result = await updateMutation.mutateAsync({ fields, expectedUpdatedAt: lastUpdatedAt });

      // Update the concurrency token with the server's new updatedAt
  ...
  }, [localFields, updateMutation, showNotification, lastUpdatedAt]);
  ...
  const isInitializing =
    ...
    ((() => {
      const fields = mappingQuery.data?.data?.fields;
