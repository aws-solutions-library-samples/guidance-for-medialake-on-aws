...
const getPermissionStatus = (permissions: any, resource: string, action: string): PermissionStatus => {
  ...
  if (Array.isArray(permissions)) {
    ...
    if (action.includes('.')) {
      const nestedPermission = permissions.find(p =>
        p.resource === resource &&
...
const PermissionMatrix: React.FC<PermissionMatrixProps> = ({
  ...
  const resources: ResourceType[] = ['assets', 'pipelines', 'pipelinesExecutions', 'collections', 'settings'];
              ...
              <TableRow key={resource} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                ...
                {getActionsForResource(resource).map(action => {
                  ...
                  if (resource === 'settings' && action === 'edit') {
                    ...
                    const hasSystemEdit = getPermissionStatus(permissions, 'settings', 'system.edit');
                    const hasRegionsEdit = getPermissionStatus(permissions, 'settings', 'regions.edit');
                    
                    if (hasUsersEdit === 'Allow' || hasSystemEdit === 'Allow' || hasRegionsEdit === 'Allow') {
                      status = 'Allow';
                    } else if (hasUsersEdit === 'Deny' && hasSystemEdit === 'Deny' && hasRegionsEdit === 'Deny') {
                      status = 'Deny';
                    } else {
                      status = 'Not Set';
                    }
                  ...
                  return (
                    <TableCell
                      key={`${resource}-${action}`}
                      align="center"
                      onClick={() => interactive && onCellClick && onCellClick(resource, action, status)}
                      sx={interactive ? {
                        cursor: 'pointer',
                        '&:hover': {
