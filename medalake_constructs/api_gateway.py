...
class ApiGatewayConstruct(Construct):
    ...
    def __init__(self, scope: Construct, id: str, user_pool, props: Optional[ApiGatewayProps]=None) -> None:
        ...
        api_handler_log_group = logs.LogGroup(self, "ApiHandlerLogGroup", removal_policy=RemovalPolicy.DESTROY)
        ...
        powertools_layer = PowertoolsLayer(
            self, "PowertoolsLayer", architecture=lambda_architecture
        )
        opensearchpy_layer = CustomLambdaLayerConstruct(
            self, "OpenSearchPy", runtime=_lambda.Runtime.PYTHON_3_12, modules=[{"name": "opensearch-py"}]
        )
        requests_aws4auth_layer = CustomLambdaLayerConstruct(
            self, "requests_aws4auth", runtime=_lambda.Runtime.PYTHON_3_12, modules=[{"name": "requests_aws4auth"}]
        )


        # Create IAM role for Lambda function
        lambda_execution_role = iam.Role(self, "LambdaRole",
            assumed_by=iam.ServicePrincipal("lambda.amazonaws.com")
        )
        ...
        twelve_labs_api_key = secretsmanager.Secret(self, 'TwelveLabsApiKei',
            secret_name='twelve_labs_api_key_secret',
            description='Twelve Labs API Key Secret',
            secret_object_value={
                "key": SecretValue.unsafe_plain_text("TEMP")
            }
        )
        ...
        api_handler = _lambda.Function(
            ...
            layers=[powertools_layer.layer,opensearchpy_layer.layer,requests_aws4auth_layer.layer],
            environment={
        ...
        # set ddb permissions
        ddb.Table.from_table_name(self,"AssetReviewTable","asset-review").grant_read_write_data(api_handler)
