...
class ApiGatewayStack(cdk.NestedStack):
    def __init__(
        ...
        # 3b. IAM grants on portal authorizer Lambda
        ...
        # 3c. API Gateway invoke permission
        ...
        self._portal_public_lambda = Lambda(
            ...
            config=LambdaConfig(
                ...
                environment_variables={
                    "SYSTEM_SETTINGS_TABLE_NAME": props.system_settings_table,
                    "MEDIALAKE_CONNECTOR_TABLE": self._connectors_api_gateway.connector_table.table_name,
                    "CLOUDFRONT_DOMAIN": props.cloudfront_domain,
                },
            ),
        )
        ...
        connector_table_arn = self._connectors_api_gateway.connector_table.table_arn
        self._portal_public_lambda.function.add_to_role_policy(
            iam.PolicyStatement(
                effect=iam.Effect.ALLOW,
                actions=["dynamodb:GetItem", "dynamodb:Query"],
