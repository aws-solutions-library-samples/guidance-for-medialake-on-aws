...
class Introspect(object):
    ...
    def __init__(self, operation_name):
        ...
        if at_least_libvips(8, 7):
            ...
            result = vips_lib.vips_object_get_args(
                op.vobject, p_names, p_flags, p_n_args
            )
            if result != 0:
...
class Operation(pyvips.VipsObject):
    ...
    def set(self, name, flags, match_image, value):
        ...
        if match_image:
            ...
            if gtype == pyvips.GValue.image_type:
                ...
                value = [pyvips.Image._imageize(match_image, x) for x in value]
    ...
    def call(operation_name, *args, **kwargs):
        ...
        if len(intro.required_input) != len(args):
            raise Error(
                f"{operation_name} needs {len(intro.required_input)} "
        ...
        for name in kwargs:
            if name not in intro.optional_input and name not in intro.optional_output:
                raise Error(
                    f"{operation_name} does not support optional " f"argument {name}"
                )
            ...
            if (details["flags"] & _DEPRECATED) != 0:
                logger.info("%s argument %s is deprecated", operation_name, name)
    ...
    def generate_docstring(operation_name):
        ...
        if (intro.flags & _OPERATION_DEPRECATED) != 0:
            raise Error(
                "No such operator.", f'operator "{operation_name}" is deprecated'
            )
        ...
        args += [x + "=bool" for x in intro.doc_optional_output]
        result += ", ".join(args) + ")\n"

        def argstr(name):
            details = intro.details[name]
            return "    {0} ({1}): {2}\n".format(
                name, GValue.gtype_to_python(details["type"]), details["blurb"]
            )
    ...
    def generate_sphinx(operation_name):
        ...
        doc_optional_input = [
            ...
            if name != "revalidate"
            or (
                "_buffer" not in operation_name
                and (intro.flags & _OPERATION_NOCACHE) == 0
            )
        ]
        ...
        for name in intro.doc_optional_output:
            result += (
                f":param {name}: " f'enable output: {intro.details[name]["blurb"]}\n'
            )
