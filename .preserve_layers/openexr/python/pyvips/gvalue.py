...
class GValue(object):
    ...
    def to_enum(gtype, value):
        ...
        if isinstance(value, str):
            enum_value = vips_lib.vips_enum_from_nick(
                b"pyvips", gtype, _to_bytes(value)
            )
            if enum_value < 0:
                raise Error(
                    "no value {0} in gtype {1} ({2})".format(
    ...
    def to_flag(gtype, value):
        ...
        if isinstance(value, str):
            flag_value = vips_lib.vips_flags_from_nick(
                b"pyvips", gtype, _to_bytes(value)
            )
            if flag_value < 0:
                raise Error(
    ...
    def set(self, value):
        ...
        if gtype == GValue.gbool_type:
            ...
            gobject_lib.g_value_set_enum(self.gvalue, GValue.to_enum(gtype, value))
        elif fundamental == GValue.gflags_type:
            gobject_lib.g_value_set_flags(self.gvalue, GValue.to_flag(gtype, value))
            ...
            if pyvips.API_mode or at_least_libvips(8, 6):
                vips_lib.vips_value_set_blob_free(self.gvalue, memory, len(value))
            else:
                vips_lib.vips_value_set_blob(
                    self.gvalue, glib_lib.g_free, memory, len(value)
                )
        else:
            raise Error(
    ...
    def get(self):
        ...
        if gtype == GValue.gbool_type:
            ...
            return GValue.from_enum(gtype, gobject_lib.g_value_get_enum(self.gvalue))
        elif fundamental == GValue.gflags_type:
            ...
            raise Error("unsupported gtype for get {0}".format(type_name(gtype)))
