...
import pyvips
from pyvips import Error, _to_bytes, at_least_libvips, ffi, gobject_lib, type_name
...
if pyvips.API_mode:
    ...
    _marshal_image_progress_cb = ffi.cast(
        "GCallback", gobject_lib._marshal_image_progress
    )
else:
    ...
    _marshal_image_progress_cb = ffi.cast("GCallback", _marshal_image_progress)
...
if at_least_libvips(8, 9):
    ...
    if pyvips.API_mode:
        ...
        _marshal_seek_cb = ffi.cast("GCallback", gobject_lib._marshal_seek)
    else:
...
class GObject(object):
    ...
    def signal_connect(self, name, callback):
        ...
        gobject_lib.g_signal_connect_data(
            go, _to_bytes(name), _marshalers[name], handle, ffi.NULL, 0
        )
