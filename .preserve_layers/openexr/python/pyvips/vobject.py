...
import pyvips
from pyvips import Error, _to_bytes, _to_string, ffi, gobject_lib, vips_lib
...
class VipsObject(pyvips.GObject):
    ...
    def _get_pspec(self, name):
        ...
        if name not in cache[class_pointer]:
            ...
            result = vips_lib.vips_object_get_argument(
                self.vobject, _to_bytes(name), pspec, argument_class, argument_instance
            )
