...
class Channel:
    ...
    def __eq__(self, other):
        return (self.type, self.xSampling, self.ySampling) == (
            other.type,
            other.xSampling,
            other.ySampling,
        )
...
class TimeCode:
    ...
    def __repr__(self):
        ...
        return (
            "<Imath.TimeCode instance { time: %s:%s:%s:%s, dropFrame: %s, colorFrame: %s, fieldPhase: %s, bgf0: %s, bgf1: %s, bgf2: %s }"
            % (
                self.hours,
                self.minutes,
                self.seconds,
...
class KeyCode:
    ...
    def __repr__(self):
        return (
            "<Imath.KeyCode instance { filmMfcCode: %s, filmType: %s, prefix: %s, count: %s, perfOffset: %s, perfsPerFrame: %s, perfsPerCount: %s }"
            % (
                self.filmMfcCode,
                self.filmType,
                self.prefix,
...
class TileDescription:
    def __init__(
        ...
        xs=32,
        ys=32,
        m=LevelMode(LevelMode.ONE_LEVEL),
        r=LevelRoundingMode(LevelRoundingMode.ROUND_DOWN),
    ):
        self.xSize = xs
        self.ySize = ys
        self.mode = m
    ...
    def __repr__(self):
        return (
            "<Imath.TileDescription instance { xSize: %s, ySize: %s, mode: %s, roundingMode: %s>"
            % (self.xSize, self.ySize, repr(self.mode), repr(self.roundingMode))
        )
